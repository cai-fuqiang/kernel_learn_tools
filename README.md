...
- **Diff 注释翻译**：代码变更区域中的 C 语言注释自动翻译，绿色斜体高亮显示
- **智能段落对齐**：基于翻译相似度评分的窗口搜索算法，确保中英文段落严格对齐
- **响应式适配**：窄屏自动切换为上下布局
...
- `v2/` — 新版输出（左右栏对比布局），包含以下增强特性：
  - 基于翻译相似度评分的智能段落对齐（窗口搜索最佳匹配）
  - 代码/数据行、ASCII 图表自动识别并跳过翻译
  - Diff 区域 C 语言注释自动翻译（绿色斜体高亮）
  - 面板可折叠、全局视图切换
